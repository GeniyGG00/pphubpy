...
class User:
    ...
    waiting_for_product_remove: bool = False
    waiting_for_quantity_update: bool = False
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(username):
        ...
        if user.waiting_for_product_remove:
            user.waiting_for_product_remove = False
        ...
        if user.waiting_for_quantity_update:
            user.waiting_for_quantity_update = False
            
            parts = text.split('|')
            if len(parts) == 2:
                try:
                    product_id, new_quantity = parts
                    updated = False
                    
                    for category_name in catalog.keys():
                        if update_product_quantity(category_name, product_id.strip(), int(new_quantity)):
                            updated = True
                            break
